...
import { useCadastroStore, FACTORIES } from '../hooks/useStore';
...
function MachinesSection() {
  const { machines, addMachine, updateMachine, removeMachine } = useCadastroStore();
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    if (modal.mode === 'add') {
      addMachine(factory, { ...form, capacity: Number(form.capacity), sides: Number(form.sides) });
    } else {
      updateMachine(factory, modal.original.id, { ...form, capacity: Number(form.capacity), sides: Number(form.sides) });
    }
    setSaving(false);
    setModal(null);
  };
  ...
  const handleRemove = (id) => {
    if (window.confirm('Remover esta máquina?')) removeMachine(factory, id);
  };
...
function ProductsSection() {
  const { products, addProduct, updateProduct, removeProduct } = useCadastroStore();
  const [modal, setModal] = useState(null);
  const [form, setForm] = useState({ id: '', name: '', type: 'DTY', dtex: 150, filaments: 48 });
  const [saving, setSaving] = useState(false);

  const openAdd = () => {
    const nextId = `P${String(products.length + 1).padStart(3, '0')}`;
    setForm({ id: nextId, name: '', type: 'DTY', dtex: 150, filaments: 48 });
    setModal({ mode: 'add' });
  };

  const openEdit = (p) => { setForm({ ...p }); setModal({ mode: 'edit', original: p }); };

  const handleSave = () => {
    if (!form.name) return;
    setSaving(true);
    if (modal.mode === 'add') addProduct({ ...form, dtex: Number(form.dtex), filaments: Number(form.filaments) });
    else updateProduct(modal.original.id, { ...form, dtex: Number(form.dtex), filaments: Number(form.filaments) });
    setSaving(false);
    setModal(null);
  };

  const handleRemove = (id) => { if (window.confirm('Remover este produto?')) removeProduct(id); };

  const TYPE_COLORS = { DTY: 'text-sky-400 bg-sky-500/10', ATY: 'text-violet-400 bg-violet-500/10', FDY: 'text-amber-400 bg-amber-500/10', POY: 'text-rose-400 bg-rose-500/10', Outro: 'text-slate-400 bg-slate-500/10' };
    ...
    <div className="bg-brand-navy border border-white/[0.06] rounded-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {products.map((p) => (
              <div key={p.id} className="flex items-center gap-3 px-4 py-3 bg-brand-slate/30 rounded-xl border border-white/[0.04] group hover:border-white/[0.08] transition-all">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-lg ${TYPE_COLORS[p.type] || TYPE_COLORS.Outro}`}>{p.type}</span>
                <div className="flex-1 min-w-0">
      ...
      {modal && (
        <Modal title={modal.mode === 'add' ? 'Novo Produto' : 'Editar Produto'} onClose={() => setModal(null)} onSave={handleSave} saving={saving}>
          <Field label="Nome">
            <input value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))}
              placeholder="DTY 150/48" className={inputCls} />
          </Field>
